...
const TestsNormal = () => {
    ...
    function ListEngWords(){
        return engs.map((element, index) => <tr  key={index}><TableTd><StyledLabel>{element}</StyledLabel></TableTd></tr>)
    }
    ...
    function PostScore(){
        ...
        huns.map(async (element, index) => {
            if(element === answers[index]){
                score += 1;
                hunwords += answers[index];
                engwords += engs[index];
            }
            else{
        ...
        fetch("http://localhost:5000/user/score", {
            ...
            .then(async res => { 
                ...
                postTest(engs, answers, score, mode);
            })
        ...
        <div>
            ...
            <StyledFormArea bg={color.dark2}>
                ...
                >
                    ...
                    <p>Írd be az angol szó/kifejezés magyar megfelelőjét</p>
